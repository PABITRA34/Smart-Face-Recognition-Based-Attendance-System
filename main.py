...
def contact():
    print("contact info hai sirf ye")
...
def check_haarcascadefile():
    ...
    exists = os.path.isfile('haarcascade_frontalface_default.xml')
    if exists:
        print("no harase code error")
        pass
    else:
        print("haracode error")
        mess._show(title='Some file missing', message='Please contact us for help')
        print("haar ne window destroy kiya")
        window.destroy()
...
#addded this block to handle with threads        
def continue_taking_images():       
        window.after(100, TakeImages)     
        window.after(100, continue_taking_images)               
...
def update_video_frame():
    ...
    if ret:
        ...
        # Update Tkinter label or canvas with the current video frame
        # ...

        window.after(1, update_video_frame)  # Schedule the function to run again
...
def TrackImages():
    ...
    # Schedule the function to update video frames
    window.after(1, update_video_frame)
